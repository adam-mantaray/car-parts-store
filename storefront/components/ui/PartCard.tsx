...
export default function PartCard({ product }: Props) {
  ...
  const priceFormatted = rateLoaded
    ? store.pricing.formatPrice(product.basePrice, 'ar')
    : null
  ...
  function handleAdd() {
    ...
    toast.success('تمت الاضافة للسلة')
  }
  ...
  return (
    <div className="card flex flex-col">
      ...
      <Link href={`/part/${oem || product._id}`}>
        ...
        >
          {image ? (
            <img src={image} alt={nameAr} className="h-full w-full object-contain p-4" />
          ) : (
      ...
      <div className="p-5 flex flex-col flex-1">
        <Link href={`/part/${oem || product._id}`} className="block mb-1 hover:text-gold transition-colors">
          <p className="font-cairo text-lg font-bold text-white">{nameAr}</p>
        </Link>
